...
int[,,] CreateMatrixRndUniqueInt(int rows, int colums, int depth, int min, int max)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < colums; j++)
        {
            ...
            {
                int shift = rnd.Next(1, len);
                count += shift;
                if (count >= len) count -= len;
                while (uniqueNumbersArray[count] == 0)
                {
                    count++;
                    if (count >= len) count -= len;
                }
...
Console.Clear();
int minRowsColumnsDepth = 2;
int maxRowsColumnsDepth = 4;
int minUniqueNumber = 10;
int maxUniqueNumber = 99;
Console.WriteLine($"Программа создает двумерных массива cо случайным количеством строк, столбцов и глубиной");
Console.WriteLine($"и заполняет случайными неповторяющимися числами.");
